...
def format_attributes(offer: ET.Element) -> str:
    parts = []
...
def parse_products(
    ...
    for offer in root.findall(".//offer"):
        ...
        attributes = format_attributes(offer)
...
def load_products(yml_path: str, db_config: dict) -> None:
    products = parse_products(yml_path)
    if not products:
    ...
    INSERT INTO products (
        offer_id, url, name, description, attributes, price, currencyId, categoryId, picture
    )
    ...
    print(f"Loaded {len(products)} products into products.")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Load YML products into MySQL for p2 prototype."
    )
